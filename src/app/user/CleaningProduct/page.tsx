...
const CleaningProduct = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const cleanninglOnly = productData.filter(
          (item: any) => item.category.toLowerCase() === "cleaning products"
        );

        setGrosary(cleanninglOnly);
      } catch (err) {
  ...
  const filterCleaningProduct = grosary.filter((item: any) =>
      ...
      <div className="mt-4">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 mb-4 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {filterCleaningProduct.map((item: any) => (
              <div
                key={item._id}
              ...
              >
                ...
                <div className="text-center  mt-4">
                  <Link href={`/user/CleaningProduct/${item._id}`}>
